...
struct Bio {
   ...
   static func createSampleData() -> Bio {
       let dateComponents = DateComponents(year: SelfPresentationStrings.BirthDate.year, month: SelfPresentationStrings.BirthDate.month, day: SelfPresentationStrings.BirthDate.day)
       let myBirthday = Calendar.current.date(from: dateComponents) ?? Date()
